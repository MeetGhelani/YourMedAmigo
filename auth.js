...
const userSignIn = async () => {
  try {
    ...
    // Set cookies
    document.cookie = `name=${encodeURIComponent(name)}; expires=${new Date(
      Date.now() + 86400000
    ).toUTCString()}; path=/`;
    document.cookie = `imgsrc=${encodeURIComponent(imgsrc)}; expires=${new Date(
      Date.now() + 86400000
    ).toUTCString()}; path=/`;
    document.cookie = `email=${encodeURIComponent(email)}; expires=${new Date(
      Date.now() + 86400000
    ).toUTCString()}; path=/`;
